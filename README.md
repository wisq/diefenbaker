...
For example, here's the contents of my `/etc/sv/dief-ups/run` file:
...
If I want to run other checks as well, I create more `/etc/sv/dief-*` services that each run one service only.  Alternatively, you can set up your `launch.sh` script to run more than one check script per invocation.

Important notes:

* If you're installing gems via your `runit` service like I am, make sure that **only one service** is doing it.
